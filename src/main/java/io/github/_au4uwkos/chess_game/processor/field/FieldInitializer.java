...
public class FieldInitializer {
    public static Field init() {
        ...
        for(int pos = 0; pos < 8; pos++){
            Pawn blackPawn = new Pawn(false,6,pos);
            Pawn whitePawn = new Pawn(true,1,pos);
            figures.put(blackPawn.getCoordinates(),blackPawn);
            figures.put(whitePawn.getCoordinates(),whitePawn);
        }
